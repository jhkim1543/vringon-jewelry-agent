...
### 이미지 비용 · 자체 호스팅으로 옮기는 길 (2026-08-19 조사)

**지금 비용.** 이미지는 장당 과금이다. 실측: 트렌드 샘플 40장 $7.60, 시리즈 34장 $6.46,
무드보드 21장 $3.99. 기본 설정(S5·스케치 12)으로 한 번 돌리면 90장 —
빠른 모델 $4.05, 디테일 모델 $17.10.

**대안으로 검토한 것.** SenseNova-U1.5-8B-MoT (Apache 2.0, 18B). 생성·편집·이해를 한 모델이
다 하므로 이 앱이 쓰는 이미지 API 와 비전 QA 를 함께 덮을 수 있다. Q4 GGUF 로 10~12GB
소비자 카드에서 돈다고 문서에 적혀 있다.

**이 개발 PC 에서는 못 돌린다.** Intel Arc 140T 내장 그래픽이고 NVIDIA 가 없다. 공식 저장소는
CUDA 만 다루고 Intel·CPU 경로가 없다. 그러므로 로컬 실행은 선택지가 아니다 —
GPU 가 있는 다른 기계나 시간당 대여가 필요하다.

**대신 갈아 끼울 수 있게 해 두었다.** 이 앱이 부르는 형태는 원래부터 OpenAI 이미지 API 다
(`/v1/images/generations`, `/v1/images/edits`, `b64_json` 응답). 같은 형태로 서빙하는 서버를
세우면 `.env` 세 줄로 옮겨간다:

```
SELF_HOST_IMAGE_URL=http://<gpu-box>:8000
SELF_HOST_IMAGE_MODEL=<그 서버가 받는 모델 이름>
SELF_HOST_IMAGE_KEY=<필요하면>
```

- `npm run selfhost:probe` 가 생성·편집 두 경로를 실제로 찔러 보고 PNG 를 `.cache/probe` 에 남긴다.
  **편집이 되는지 반드시 볼 것** — 이 앱은 뷰·컬러웨이·베리에이션·착용컷을 전부 기준 렌더의
  편집으로 만든다. 생성만 되는 서버는 반쪽만 돌아간다.
- 켜지면 **폴백하지 않는다.** 실패하면 그대로 실패한다. 비용을 아끼려고 켠 것인데 조용히
  유료 경로로 새면 켠 의미가 없기 때문이다.
- 캐시 키에 백엔드 이름이 들어간다(`self:<model>`). 빼면 백엔드를 바꾼 뒤에도 예전 유료 API
  결과가 나와 무엇을 보고 있는지 알 수 없다.
- 화면에는 공급사명을 쓰지 않는다. "직접 세운 이미지 서버" 로만 표시한다.

**아직 확인하지 못한 것.** 실제 GPU 가 없어 이 경로로 이미지를 만들어 본 적이 없다.
vLLM-Omni 가 SenseNova-U1 을 OpenAI 호환으로 서빙한다고 문서에 있으나, 이미지 엔드포인트를
여는지 채팅만 여는지는 확인하지 못했다. 공식 저장소가 권하는 배포는 LightLLM + LightX2V 도커이고
그쪽 엔드포인트 형태는 문서에 없다. **GPU 를 확보하면 프로브부터 돌릴 것.**
프로브가 실패하면 `server/openai-api.mjs` 의 `selfHostImage()` 하나만 그 서버의 형태에 맞추면 된다.
